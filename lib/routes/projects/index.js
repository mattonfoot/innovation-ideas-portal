...
const createProjectListModel = () => (r) => ({
    ...
    published: map(r[6], (x) => new ProjectViewModel(x)),
  });
...
const reviewBasicDetails = (req, res, next) =>
  dbfind({ _id: req.params.id, type: 'in draft' })
...
const displayProjectList = (req, res, next) =>
  Promise.all([
    dbfind({ status: 'in draft' }),
    dbfind({ status: 'triaged' }),
    dbfind({ status: 'future' }),
    dbfind({ status: 'wip' }),
